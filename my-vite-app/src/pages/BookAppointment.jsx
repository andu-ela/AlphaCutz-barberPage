...
const BookAppointment = () => {
  ...
  const fetchAppointments = async (date) => {
    try {
      const response = await fetch(
        `http://localhost:5000/api/reserved-times?date=${date}`
      );
  ...
  const filteredTimes = availableTimes.filter(
    (time) => !appointments.includes(time.value)
  );
                ...
                return (
                  <div
                    ...
                    className={`time-button ${
                      time.value === selectedTime ? "selected" : ""
                    } ${isReserved ? "reserved" : ""}`}
            ...
            {["Men's Haircut", "Beard Trim"].map((serviceName) => (
              <button
                ...
                className={`service-label ${
                  service === serviceName ? "active-service" : ""
                }`}
